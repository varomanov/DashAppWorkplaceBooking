from dash import register_page, html, callback, Input, Output, State, no_update, dcc
...
layout = dbc.Container([
   ...
   html.Div([
        ...
        dbc.Select(
            ...
            placeholder='Дата',
            options=[{'label': '🟢'.join(['', x.strftime('%Y-%m-%d')]), 'value': x} for x in get_dates()]
        ),
        html.P('🪧 Место', className='mt-5 mb-2 ms-1'),
        dbc.Select(
            id='booking-select-place',
            placeholder='Место',
            options=[{'label': ' '.join(['✨', x['name']]), 'value': x['id']} for x in get_places()]
        ),
        html.Br(),
        html.Br(),
        html.Hr(),
        html.Div(dbc.Button('Забронировать', id='booking-button-book', disabled=True, color='secondary'), className='text-center my-5'),
        html.Div(id='boking-alert')
   ], className='p-3 my-5 border', style={'backgroundColor': '#eee', 'borderRadius': '15px'}),
